...
class UserDataSetsView(UsersDataPermission, MultipleFieldLookupMixin, generics.ListCreateAPIView, mixins.DestroyModelMixin):
    ...
    def get_object(self):
        ...
        # type: ignore
        return Dataset.objects.filter(name=self.request.query_params['dataset_name'])
...
class UserDataSetView(UsersDataPermission, generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        for obj in new_objs:
            if DataPair.objects.filter(dataset=dataset, key=obj["key"]).exists():
                ...
                continue
            to_add.append(
                DataPair(key=obj["key"], value=obj["value"], dataset=dataset))
    ...
    def get_other_users(self):
        ...
        other_users = SetToUser.objects.filter(dataset=dataset).select_related(
            "user").values("permission", "user__email")
...
def manage_dataset_users(request, *args, **kwargs):
    ...
    try:
        ...
        raise NotFound(
            f"{request.user} does not have access to {dataset.name}")
...
def get_dataset_users(request, *args, **kwargs):
    ...
    if set_to_user.permission == 'admin':
        other_users = SetToUser.objects.filter(dataset=dataset).select_related(
            "user").values("can_read", "can_write", "can_admin", "user__email")
        ...
        raise PermissionDenied(
            f"{request.user} does not have necessary permissions for {dataset.name}")
...
class SetToUserView(generics.RetrieveUpdateDestroyAPIView, mixins.CreateModelMixin, mixins.ListModelMixin):
    ...
    def put(self, request, *args, **kwargs):
        ...
        try:
            target_set_to_user = SetToUser.objects.get(
                dataset=self.get_dataset(), user=target_user)
        except SetToUser.DoesNotExist:
            raise NotFound()
